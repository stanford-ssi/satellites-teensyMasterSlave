...
// This only exists for the DmaSpi library; this class is never instantiated
class SPIClass {
public:
...
public:
	...
	inline static void beginTransaction(SPISettings settings) {
		...
		#if 0
        // Ignore SPI_TRANSACTION_MISMATCH_LED
		if (inTransactionFlag) {
			pinMode(SPI_TRANSACTION_MISMATCH_LED, OUTPUT);
			digitalWrite(SPI_TRANSACTION_MISMATCH_LED, HIGH);
		}
	...
	inline static uint8_t transfer(uint8_t data) {
		SPI2_SR = SPI_SR_TCF;
		SPI2_PUSHR = data;
		for (int loopCounter = 0; !(SPI2_SR & SPI_SR_TCF); loopCounter++) {
            if (loopCounter > 100000) {
                Serial.println("Error: loop uint8_t failed");
                break;
            }
        }
		return SPI2_POPR;
	...
	inline static uint16_t transfer16(uint16_t data) {
		...
		for (int loopCounter = 0; !(SPI2_SR & SPI_SR_TCF); loopCounter++) {
            if (loopCounter > 100000) {
                Serial.println("Error: loop uint16_t failed");
                break;
            }
        }
		return SPI2_POPR;
...
// This module is reserved for the Dma library
extern SPIClass SPI;
//extern SPIClass SPI;
